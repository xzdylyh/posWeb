...
class ChargePage(basepage.BasePage):
    ...
    # 确定按钮
    charge_confirmBtn_loc = (
        By.CSS_SELECTOR,
        'div.panel-body.charge-number > div > div > button'
    )

    # 储值奖历规则
    charge_GZ_loc = (By.CSS_SELECTOR,'div > div.tag.active > p:nth-child(2)')
    # 自定义储值规则
    charge_customGZ_loc = (By.LINK_TEXT,'自定义规则')
    # 自定义输入金额
    charge_present_loc = (By.ID,'present')
    # 确定按钮
    charge_customBtn_loc = (By.XPATH,'//input[@id="present"]/../div[1]')
    # 支付类型
    charge_payType_loc =(
        By.CSS_SELECTOR,
        'div.pay-out-box > div > label:nth-child(1)'
    )
    # 备注
    ...
    charge_RMB_loc = (
        By.CSS_SELECTOR,
        'div.panel-body > div > div:nth-child(2) > div'
    )
    # 储值最后确认
    charge_LastBtn_loc = (By.ID,'chargeCheckBtn')
    # 储值成功提示
    assertChargeSuccess_loc = (
        By.XPATH,
        '//*[@id="chargeSuccessModal"]/div/div/div[1]/h3'
    )
    # 立即消费按钮
    ...
    fill_RMB_loc = (
        By.XPATH,
        '//*[@id="receipt"]/div[2]/div[2]/div[2]/div[1]/div[5]/input'
    )
    ...
    fill_not_RMB_loc = (
        By.XPATH,
        '//*[@id="receipt"]/div[2]/div[2]/div[2]/div[1]/div[4]'
    )
    # 确定按钮
